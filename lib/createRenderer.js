...
module.exports = function createRenderer(opt) {
  ...
  const noise3D = createNoise3D(randFunc);
  var ctx = opt.context;
  ...
  var time = 0;
  var particles = Array.from({ length: count }, () => resetParticle());
  ...
  function step(dt) {
    ...
    particles.forEach((p, i) => {
      ...
      var n = noise3D(fx * pS, fy * pS, p.duration + time);
      ...
      var s2 = pointilism;
      var r = p.radius * noise3D(x * s2, y * s2, p.duration + time);
